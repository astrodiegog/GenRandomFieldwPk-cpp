...
extern void set_real_random_field(int global_seed, PS_Params *ps_params,  ptrdiff_t alloc_local, double *xi_arr_local)
{
    int i;
    ...
    if (procID == rand_generator){
        for (i = 0; i < alloc_local ; i++)
        {
            xi_arr_local[i] = normal_dist(eng);
        }
        printf("--- Rank %d : done generating random numbers \n", procID);
    }
